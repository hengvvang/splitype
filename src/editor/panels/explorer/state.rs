//! ExplorerState sidebar model — file-tree scanning and flat visible entry
//! list.
//!
...
//!
//! The outline panel has its own state in
//! `crate::editor::panels::outline`; explorer interactions and rendering
//! live in `crate::explorer`.
...
/// Which file-tree item is currently selected in the explorer sidebar.
///
/// Files are keyed by the Zed-style double key `(root, entry)`: the
/// worktree index plus the worktree-allocated stable entry id. Outline
