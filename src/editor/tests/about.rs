...
fn about_dialog_body_lines_include_repository_and_star_message() {
    ...
    assert_eq!(
        lines[2],
        format!("GitHub: {}", crate::editor::panes::document_pane::SPLITYPE_REPOSITORY_URL)
    );
    assert_eq!(
        lines[3],
...
async fn about_github_link_uses_gpui_url_opening(cx: &mut TestAppContext) {
    cx.update(|cx| {
        crate::editor::panes::document_pane::open_splitype_repository(cx);
    });

    assert_eq!(
        cx.opened_url(),
        Some(crate::editor::panes::document_pane::SPLITYPE_REPOSITORY_URL.to_string())
    );
}
