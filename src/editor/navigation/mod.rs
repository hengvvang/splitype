...
mod tests {
    ...
    #[test]
    fn preview_navigation_resolves_directly() {
        let intent = NavigationIntent::new(
            NavigationTarget::External {
                raw: "https://splitype.dev".into(),
                resolved: "https://splitype.dev".into(),
            },
            NavigationMode::Preview,
            Modifiers::default(),
        );
        assert_eq!(
            intent.resolve_policy(),
            Some(NavigationExecutionPlan::OpenExternalUrl("https://splitype.dev".into()))
        );
    }

    #[test]
    fn wysiwyg_requires_secondary_modifier_for_links() {
        let plain_intent = NavigationIntent::new(
            NavigationTarget::External {
                raw: "https://splitype.dev".into(),
                resolved: "https://splitype.dev".into(),
            },
            NavigationMode::Wysiwyg,
            Modifiers::default(),
        );
        assert_eq!(plain_intent.resolve_policy(), None);
        ...
        let ctrl_intent = NavigationIntent::new(
            NavigationTarget::External {
                raw: "https://splitype.dev".into(),
                resolved: "https://splitype.dev".into(),
            },
        ...
        assert_eq!(
            ctrl_intent.resolve_policy(),
            Some(NavigationExecutionPlan::PromptAndOpenExternalUrl {
                prompt_target: "https://splitype.dev".into(),
                open_target: "https://splitype.dev".into(),
            })
        );
    }

    #[test]
    fn wysiwyg_footnote_jumps_without_modifier() {
        let intent = NavigationIntent::new(
            NavigationTarget::FootnoteDefinition { id: "note1".into() },
            NavigationMode::Wysiwyg,
            Modifiers::default(),
        );
        assert_eq!(
            intent.resolve_policy(),
            Some(NavigationExecutionPlan::JumpToFootnoteDefInEditor("note1".into()))
        );
    }
}
