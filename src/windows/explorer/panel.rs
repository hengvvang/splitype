...
use crate::windows::explorer::worktree::Worktree;
...
impl Editor {
    ...
    pub(crate) fn add_explorer_worktree(&mut self, path: PathBuf, cx: &mut Context<Self>) {
        ...
        let worktree = Worktree::new(path.clone(), explorer.next_entry_id.clone(), hide_hidden, cx);
    ...
    pub(crate) fn move_explorer_worktree(&mut self, from: usize, to: usize, cx: &mut Context<Self>) {
        let explorer = &mut self.panels.explorer;
    ...
    pub(crate) fn delete_explorer_item(
        ...
        let strings = cx.global::<crate::infra::i18n::I18nManager>().strings().clone();
        let ok_btn = "Delete";
