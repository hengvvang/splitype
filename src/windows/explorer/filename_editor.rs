...
impl Editor {
    ...
    fn explorer_duplicate_name_error(
        ...
        let existing = crate::windows::explorer::state::find_explorer_node(tree, &new_path);
        let is_self = edit
            .target_id
    ...
    pub(crate) fn begin_explorer_rename(
        ...
        let selection_end = if node.kind == crate::windows::explorer::state::ExplorerEntryKind::Directory
        {
            file_name.len()
        } else {
            target_path
