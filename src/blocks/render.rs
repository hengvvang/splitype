...
use super::{
    Block, BlockAction, BlockType, ImageResolvedSource, ImageHandle, code_language_display_name,
    code_language_options_matching,
};
use crate::blocks::{
    HtmlCssColor, HtmlDocument, HtmlNode, HtmlNodeKind, InlineScript, TableAxisHighlight,
...
use crate::workspace::{Theme, ThemeDimensions, ThemeManager};
    ...
    fn render_math_content(&self, theme: &Theme) -> AnyElement {
        ...
        let source = parse_display_math_source(raw).unwrap_or_else(|| {
            let body = raw
                ...
                .trim()
                .to_string();
            crate::blocks::DisplayMathSource {
                raw: raw.to_string(),
                body,
            }
        });
    ...
    fn render_inline_tree_children(
        ...
        for span in cache.spans() {
            if cursor < span.range.start {
                let fallback_span = crate::blocks::InlineSpan {
                    range: cursor..span.range.start,
                    style: crate::blocks::InlineStyle::default(),
                    html_style: None,
                    link: None,
                    footnote: None,
                    math: None,
            ...
            match segment {
                ...
                TableCellInlineImageSegment::Image { markdown, syntax } => {
                    if let Some(runtime) = self.image_runtime_for_syntax(syntax) {
                        ...
                        let tree = crate::blocks::RichText::plain(markdown);
                        children.extend(self.render_inline_tree_children(
                            &tree,
                            theme,
                            theme.colors.text_default,
...
impl Render for Block {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        if self.is_table_cell() {
            ...
            let style_as_header =
                is_header && crate::workspace::EditorSettings::show_table_headers(cx);
        ...
        let content = match self.kind() {
            ...
            BlockType::Heading { level: 1 } => focused_base
                ...
                .into_any_element(),
            BlockType::Heading { level: 2 } => focused_base
                ...
                .into_any_element(),
            BlockType::Heading { level: 3 } => focused_base
                ...
                .into_any_element(),
            BlockType::Heading { level: 4 } => focused_base
                ...
                .into_any_element(),
            BlockType::Heading { level: 5 } => focused_base
                ...
                .into_any_element(),
            BlockType::Heading { level: 6 } => focused_base
            ...
            BlockType::NumberedListItem => focused_base
                .text_size(px(t.text_size))
                .text_color(c.text_default)
                .line_height(rems(t.text_line_height))
                .w_full()
            ...
            BlockType::Table => {
                ...
                let row_button_hovered = self.table_append_row_button_hovered;
                ...
                let header_marker = crate::blocks::TableAxisMarker {
                    kind: TableAxisKind::Row,
                    index: 0,
                };
                ...
                let header_row = div()
                    ...
                    .children(header_cells.into_iter().enumerate().map(|(column, cell)| {
                        ...
                        let col_marker = crate::blocks::TableAxisMarker {
                        ...
                        div()
                            .id(ElementId::Name(
                                format!("table-header-cell-wrap-{}-{}", self.record.id, column)
                                    .into(),
                            ))
                            .relative()
                            .flex_none()
                ...
                let body_rows =
                    runtime
                        ...
                        .map(|(body_row_index, row)| {
                            ...
                            let marker = crate::blocks::TableAxisMarker {
                                kind: TableAxisKind::Row,
                                index: visual_row,
                            };
...

                {
                    ...
                    let column_control = div()
                        ...
                        .child(
                            div()
                                ...
                                .child(
                                    svg().path("icon/table/plus.svg").size(px(10.0)).text_color(
                    ...
                    let expand_control = div()
                        ...
                        .block_mouse_except_scroll()
                        .on_click(cx.listener(Self::on_expand_table))
                        .child(
                            svg()
            ...
            BlockType::HtmlBlock => {
                let html = self.record.html.as_ref().cloned().unwrap_or_else(|| {
                    crate::blocks::parse_html_document(
                        self.record
                            .raw_fallback
                            .as_deref()
                            .unwrap_or_else(|| self.display_text()),
            ...
            | BlockType::Heading { .. } => focused_base
                .text_size(px(t.text_size))
                .text_color(c.text_default)
                .line_height(rems(t.text_line_height))
                .child(self.render_text_or_mixed_inline_visuals(
...
mod tests {
    ...
    use crate::blocks::{Block, BlockType, BlockData, RichText, parse_html_document};
    use crate::blocks::{TableAxisKind, TableAxisMarker};
