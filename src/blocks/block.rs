...
use crate::blocks::html::{HtmlDocument, parse_html_document};
use crate::blocks::image::syntax::parse_standalone_image;
use crate::blocks::inline::text::RichText;
use crate::blocks::{TableAxisKind, TableData};
...
impl BlockData {
    ...
    pub fn raw_markdown(markdown: impl Into<String>) -> Self {
        ...
        let mut record = Self::with_plain_text(BlockType::RawMarkdown, markdown.clone());
    ...
    pub fn comment(markdown: impl Into<String>) -> Self {
        let markdown = markdown.into();
        let mut record = Self::with_plain_text(BlockType::Comment, markdown.clone());
        record.raw_fallback = Some(markdown);
        record
    }
    ...
    pub fn html(markdown: impl Into<String>) -> Self {
        ...
        let mut record = Self::with_plain_text(BlockType::HtmlBlock, markdown.clone());
    ...
    pub fn math(markdown: impl Into<String>) -> Self {
        let markdown = markdown.into();
        let mut record = Self::with_plain_text(BlockType::MathBlock, markdown.clone());
        record.raw_fallback = Some(markdown);
        record
    }
    ...
    pub fn mermaid(markdown: impl Into<String>) -> Self {
        let markdown = markdown.into();
        let mut record = Self::with_plain_text(BlockType::MermaidBlock, markdown.clone());
        record.raw_fallback = Some(markdown);
        record
    }
