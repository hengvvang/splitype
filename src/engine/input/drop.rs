...
use crate::engine::editor::{Editor, EditMode};
...
impl Editor {
    ...
    pub(crate) fn replace_after_successful_save(&mut self, window: &mut Window, cx: &mut Context<Self>) {
    ...
    pub(crate) fn save_via_prompt_then_replace_drop(&mut self, window: &mut Window, cx: &mut Context<Self>) {
    ...
    pub(crate) fn replace_after_successful_save_async(&mut self, cx: &mut Context<Self>) -> Result<()> {
        let Some(drop_path) = self.pending_drop_replace_path.take() else {
            self.clear_pending_drop_replace_state(cx);
            return Ok(());
        };
