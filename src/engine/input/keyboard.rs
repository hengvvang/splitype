//! Editor-side handling for [`BlockAction`] values emitted by child blocks.
//!
...
//! tree mutations are delegated to [`Document`](crate::engine::document::Document)
...
use crate::blocks::{
    BlockAction, BlockType, BlockData, CollapsedCaretAffinity, IndentBlock, RichText,
    OutdentBlock, PastedImageSource, TableCellPosition, is_table_row_candidate,
    parse_root_table_region, parse_table_body_row,
};
use crate::workspace::{ImagePasteBehavior, read_app_settings};
...
impl Editor {
    pub(crate) fn focused_block_for_tab_key(
        ...
        let is_focused = |block: &Entity<crate::blocks::Block>| {
    ...
    pub(crate) fn block_is_quote_structure_related(&self, block: &Entity<crate::blocks::Block>, cx: &App) -> bool {
    ...
    pub(crate) fn rendered_quote_text_requires_reparse(block: &Entity<crate::blocks::Block>, cx: &App) -> bool {
        let block_ref = block.read(cx);
    ...
    pub(crate) fn reset_block_cursor(block: &Entity<crate::blocks::Block>, cursor: usize, cx: &mut Context<Self>) {
    ...
    pub(crate) fn show_image_paste_error(&self, err: anyhow::Error, cx: &mut Context<Self>) {
        let strings = cx
            .global::<crate::workspace::I18nManager>()
            .strings()
            .clone();
    ...
    pub(crate) fn inserted_image_tree_for_block(block: &crate::blocks::Block, markdown: &str) -> RichText {
    ...
    pub(crate) fn handle_paste_image_request(
        ...
        if self.replace_cross_block_selection_with_text(
            ...
            None,
            false,
            crate::blocks::UndoCaptureKind::NonCoalescible,
            cx,
        ) {
            return;
        }

        self.prepare_undo_capture(crate::blocks::UndoCaptureKind::NonCoalescible, cx);
    ...
    pub(crate) fn is_setext_heading_target(block: &Entity<crate::blocks::Block>, cx: &App) -> bool {
        let block = block.read(cx);
    ...
    pub(crate) fn extend_table_with_typed_row(
        &mut self,
        table_block: &Entity<crate::blocks::Block>,
        row_block: &Entity<crate::blocks::Block>,
        text: &str,
        cx: &mut Context<Self>,
    ) -> bool {
    ...
    pub(crate) fn quote_break_insertion_target(
        ...
        entity_id: EntityId,
        cx: &App,
    ) -> Option<(Option<Entity<crate::blocks::Block>>, usize)> {
    ...
    pub(crate) fn ensure_callout_body_entry(
        &mut self,
        callout: &Entity<crate::blocks::Block>,
        cx: &mut Context<Self>,
    ) -> Option<Entity<crate::blocks::Block>> {
    ...
    pub(crate) fn materialize_empty_callout_shortcut(
        ...
        let Some((variant, title)) =
            crate::blocks::CalloutVariant::parse_header_line(&title_markdown)
        else {
            return None;
        };
        ...
        block.update(cx, |block, cx| {
            ...
            block
                .record
                .set_title(RichText::from_markdown(&title));
            block.sync_edit_mode_from_kind();
            block.sync_render_cache();
            block.cursor_blink_epoch = Instant::now();
            cx.notify();
...
mod tests {
    use super::Editor;
    use crate::blocks::{
        Block, BlockAction, BlockType, BlockData, CalloutVariant, Delete, DeleteBack,
        ExitCodeBlock, RichText, Newline,
    };
    ...
    async fn raw_like_block_exit_shortcut_creates_plain_text_block(cx: &mut TestAppContext) {
        ...
        let cases = [
            ...
            (
                BlockData::mermaid("```mermaid\nflowchart LR\nA-->B\n```"),
                BlockType::MermaidBlock,
                "```mermaid\nflowchart LR\nA-->B\n```",
            ),
            (
                BlockData::raw_markdown("::: custom\ncontent\n:::"),
    ...
    async fn quote_newline_inside_title_stays_in_one_source_authoritative_group(
        ...
        editor.update(cx, |editor, cx| {
            ...
            quote.update(cx, |block, cx| {
                block.prepare_undo_capture(crate::blocks::UndoCaptureKind::NonCoalescible, cx);
