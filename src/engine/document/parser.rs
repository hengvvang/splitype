...
use crate::blocks::{
    BlockType, BlockData, CalloutVariant, CodeFenceOpening, RichText,
    parse_footnote_definition_head,
};
use crate::blocks::{HtmlSafetyClass, parse_html_document};
use crate::blocks::{
...
use crate::blocks::{is_mermaid_info_string, parse_display_math_source};
...
fn parse_opening_fence(line: &str) -> Option<FenceInfo> {
    BlockType::parse_code_fence_opening(strip_fence_indent(line)?.trim_end())
}
...
fn parse_list_marker(line: &str) -> Option<ListMarker> {
        ...
        && matches!(marker, '-' | '*' | '+')
    {
        ...
        let (kind, text) =
            if let Some((checked, prefix_len)) = BlockType::parse_task_list_item_prefix(text) {
                (
                    BlockType::TaskListItem { checked },
                    text[prefix_len..].to_string(),
                )
            } else {
                (BlockType::BulletedListItem, text.to_string())
            };
    ...
    Some(ListMarker {
        kind: BlockType::NumberedListItem,
        indent_columns,
...
fn quote_content_starts_unsupported(lines: &[String], index: usize) -> bool {
    ...
    is_block_html_start(line)
        ...
        || BlockType::parse_atx_heading_line(line).is_some()
        || BlockType::parse_separator_line(line)
        || lines
            .get(index + 1)
            .and_then(|next| BlockType::parse_setext_underline(next))
            .is_some()
}
...
fn collect_unsupported_quote_region(lines: &[String], start: usize) -> Option<usize> {
    ...
    if BlockType::parse_atx_heading_line(line).is_some() || BlockType::parse_separator_line(line) {
        return Some(start + 1);
    }
    if lines
        .get(start + 1)
...
fn attach_child_blocks(
    parent: &Entity<crate::blocks::Block>,
    children: Vec<Entity<crate::blocks::Block>>,
...
fn collect_fenced_code_block(
    ...
    lines: &[String],
    start: usize,
) -> Option<(Entity<crate::blocks::Block>, usize)> {
    ...
    if is_mermaid_info_string(fence.language.as_ref().map(|language| language.as_ref())) {
        ...
        return Some((
            Editor::new_block(cx, BlockData::mermaid(raw)),
            closing_index + 1,
        ));
    }
...
fn raw_block(cx: &mut Context<Editor>, markdown: String) -> Entity<crate::blocks::Block> {
    Editor::new_block(cx, BlockData::raw_markdown(markdown))
}

fn comment_block(cx: &mut Context<Editor>, markdown: String) -> Entity<crate::blocks::Block> {
    Editor::new_block(cx, BlockData::comment(markdown))
}

fn html_or_raw_block(cx: &mut Context<Editor>, markdown: String) -> Entity<crate::blocks::Block> {
    let document = parse_html_document(&markdown);
    if document.safety == HtmlSafetyClass::Semantic {
        let mut record = BlockData::html(markdown);
...
fn math_or_raw_block(cx: &mut Context<Editor>, markdown: String) -> Entity<crate::blocks::Block> {
    if parse_display_math_source(&markdown).is_some() {
        Editor::new_block(cx, BlockData::math(markdown))
    } else {
        raw_block(cx, markdown)
    }
}
...
fn standalone_image_block(
    ...
    Editor::new_block(cx, BlockData::paragraph(markdown.trim().to_string()))
}
...
fn append_markdown_to_block(
    ...
    block.update(cx, |block, _cx| {
        ...
        if !separator.is_empty() {
            title.append_tree(RichText::plain(separator.to_string()));
        }
        title.append_tree(RichText::from_markdown(markdown));
...
fn plain_text_paragraph_block(
    cx: &mut Context<Editor>,
    text: String,
) -> Entity<crate::blocks::Block> {
    Editor::new_block(cx, BlockData::paragraph(text))
}

fn append_quote_separator_children(
    children: &mut Vec<Entity<crate::blocks::Block>>,
    count: usize,
    cx: &mut Context<Editor>,
) {
    for _ in 0..count {
        children.push(native_block(cx, BlockType::Paragraph, String::new()));
    }
}
...
fn build_native_footnote_definition_block(
    cx: &mut Context<Editor>,
    lines: &[String],
) -> Option<Entity<crate::blocks::Block>> {
    ...
    let block = Editor::new_block(
        cx,
        BlockData::new(BlockType::FootnoteDefinition, RichText::plain(id)),
    );
...
impl Editor {
    ...
    /// [`BlockType::RawMarkdown`] so they are preserved exactly on save.
        ...
        while index < lines.len() {
            ...
            if line.trim().is_empty() {
                ...
                let previous_root_is_list_item = roots
                    .last()
                    .map(|block: &Entity<crate::blocks::Block>| {
                        block.read(cx).kind().is_list_item()
                    })
                    .unwrap_or(false);
                ...
                for _ in 0..preserved_empty_blocks {
                    roots.push(native_block(cx, BlockType::Paragraph, String::new()));
                }
                continue;
            }
            ...
            if let Some((level, content)) = BlockType::parse_atx_heading_line(line) {
                roots.push(native_block(cx, BlockType::Heading { level }, content));
                index += 1;
                continue;
            }

            if BlockType::parse_separator_line(line) {
                roots.push(Self::new_block(
                    cx,
                    BlockData::new(
                        BlockType::Separator,
                        RichText::plain(line.to_string()),
                    ),
                ));
                index += 1;
                continue;
            ...
            if is_root_table_candidate_line(line) {
                ...
                if let Some(table) = parse_root_table_region(region) {
                    roots.push(Self::new_block(cx, BlockData::table(table)));
                } else {
                    roots.extend(
                        region
                            .iter()
    ...
    pub(crate) fn collect_paragraph_block(
        ...
        lines: &[String],
        start: usize,
    ) -> (Entity<crate::blocks::Block>, usize) {
        ...
        (
            native_block(cx, BlockType::Paragraph, paragraph_lines.join("\n")),
            index,
        )
    }
    ...
    pub(crate) fn build_native_quote_block(
        ...
        while index < lines.len() {
            ...
            if is_table_candidate_line(line) {
                ...
                if let Some(table) = parse_table_region(table_region) {
                    children.push(Self::new_block(cx, BlockData::table(table)));
                } else {
            ...
            if is_quote_start(line) {
                ...
                if quote.read(cx).kind() == BlockType::RawMarkdown {
                    return None;
                }
                children.push(quote);
                saw_child = true;
            ...
            if parse_list_marker(line).is_some() {
                ...
                if list_blocks
                    .iter()
                    .any(|block| block.read(cx).kind() == BlockType::RawMarkdown)
                {
                    return None;
                }
        ...
        let block = native_block(cx, BlockType::Quote, title_markdown);
        attach_child_blocks(&block, children, cx);
        Some(block)
    }
    ...
    pub(crate) fn collect_list_blocks(
        ...
        lines: &[String],
        start: usize,
    ) -> (Vec<Entity<crate::blocks::Block>>, usize) {
        let mut roots = Vec::new();
        let mut index = start;
...
mod tests {
    ...
    use crate::blocks::{BlockType, CalloutVariant, Editor, HtmlCssColor};

    #[test]
    pub(crate) fn closing_fence_must_match_exact_opening_run_length() {
    ...
    #[test]
    pub(crate) fn fence_detection_rejects_indent_beyond_three_spaces() {
    ...
    #[test]
    pub(crate) fn unmatched_opening_fence_does_not_form_code_block() {
        let lines = vec![
            "```rust".to_string(),
            "fn main() {}".to_string(),
    ...
    #[test]
    pub(crate) fn matching_closing_fence_can_skip_inner_non_closing_backtick_runs() {
        let lines = vec![
            "```rust".to_string(),
            "````".to_string(),
            "body".to_string(),
    ...
    #[test]
    pub(crate) fn fence_closes_at_first_match_even_before_a_later_opener() {
    ...
    #[test]
    pub(crate) fn empty_language_fence_closes_at_first_match() {
    ...
    #[test]
    pub(crate) fn info_tagged_fence_does_not_absorb_following_empty_blocks() {
    ...
    #[test]
    pub(crate) fn next_opening_without_prior_closing_leaves_fence_unmatched() {
        let lines = vec![
            "```rust".to_string(),
            "body".to_string(),
            "```ts".to_string(),
    ...
    #[test]
    pub(crate) fn parses_indented_code_blocks() {
    ...
    #[test]
    pub(crate) fn parses_original_unordered_list_markers() {
        assert_eq!(
    ...
    #[test]
    pub(crate) fn strips_one_quote_level_per_line() {
    ...
    #[test]
    pub(crate) fn recognizes_reference_definition_lines() {
    ...
    #[test]
    pub(crate) fn block_html_region_runs_until_blank_line() {
        let lines = vec![
            "<table>".to_string(),
    ...
    async fn preserves_chinese_spaces_in_paragraph_round_trip(cx: &mut TestAppContext) {
        ...
        editor.update(cx, |editor, cx| {
            ...
            assert_eq!(editor.document.to_markdown(cx), chinese);
        });
    }
    ...
    async fn imports_parenthesized_ordered_lists_and_serializes_canonical_dot_markers(
        ...
        editor.update(cx, |editor, cx| {
            ...
            assert_eq!(visible[0].entity.read(cx).list_ordinal, Some(1));
            assert_eq!(visible[1].entity.read(cx).list_ordinal, Some(2));
            assert_eq!(editor.document.to_markdown(cx), "1. one\n2. two");
        });
    }
    ...
    async fn imports_nested_parenthesized_ordered_list_children(cx: &mut TestAppContext) {
        ...
        editor.update(cx, |editor, cx| {
            ...
            assert_eq!(editor.document.to_markdown(cx), "1. parent\n  1. child");
        });
    }
    ...
    async fn imports_nested_quotes_as_native_blocks(cx: &mut TestAppContext) {
        ...
        editor.update(cx, |editor, cx| {
            ...
            assert_eq!(visible[2].entity.read(cx).kind(), BlockType::Quote);
    ...
    async fn imports_important_callout_case_insensitively(cx: &mut TestAppContext) {
        ...
        editor.update(cx, |editor, cx| {
            ...
            assert_eq!(
                visible[0].entity.read(cx).kind(),
                BlockType::Callout(CalloutVariant::Important)
            );
    ...
    async fn imports_callout_title_and_nested_quote_child(cx: &mut TestAppContext) {
        ...
        editor.update(cx, |editor, cx| {
            ...
            assert_eq!(
                visible[0].entity.read(cx).kind(),
                BlockType::Callout(CalloutVariant::Warning)
            );
    ...
    async fn unknown_callout_marker_stays_plain_quote(cx: &mut TestAppContext) {
        ...
        editor.update(cx, |editor, cx| {
            ...
            assert_eq!(editor.document.to_markdown(cx), "> [!UNKNOWN]");
        });
    }
    ...
    async fn preserves_separator_between_quote_title_and_nested_child(cx: &mut TestAppContext) {
        ...
        editor.update(cx, |editor, cx| {
            ...
            assert_eq!(visible[2].entity.read(cx).quote_depth, 2);
            assert_eq!(editor.document.to_markdown(cx), "> outer\n> \n> > inner");
        });
    }
    ...
    async fn imports_quote_with_native_table_child(cx: &mut TestAppContext) {
        ...
        editor.update(cx, |editor, cx| {
            ...
            assert_eq!(visible[1].entity.read(cx).kind(), BlockType::Table);
            assert_eq!(visible[1].entity.read(cx).quote_depth, 1);
            let table = visible[1]
                .entity
                .read(cx)
    ...
    async fn invalid_table_inside_quote_preserves_outer_quote_and_raw_child(
        ...
        editor.update(cx, |editor, cx| {
            ...
            assert_eq!(visible[1].entity.read(cx).kind(), BlockType::RawMarkdown);
            assert_eq!(
                visible[1].entity.read(cx).display_text(),
                "| A |\n| --- | --- |\n| 1 |"
            );
    ...
    async fn final_mixed_mega_block_preserves_important_callout_with_native_table_and_native_footnote(
        ...
        editor.update(cx, |editor, cx| {
            ...
            assert!(visible.iter().any(|visible| {
                ...
                block.kind() == BlockType::BulletedListItem && block.quote_depth == 1
            }));
            assert!(visible.iter().any(|visible| {
                let block = visible.entity.read(cx);
                block.kind()
            ...
            assert!(visible.iter().any(|visible| {
                let block = visible.entity.read(cx);
                block.kind() == BlockType::FootnoteDefinition
            ...
            assert!(
                visible
                    ...
                    .filter(|visible| {
                        let block = visible.entity.read(cx);
                        block.kind() == BlockType::BulletedListItem
                            && block.footnote_anchor.is_some()
                            && block.quote_depth == 1
                    })
                    .count()
    ...
    async fn unsupported_nested_block_preserves_native_list_item_with_raw_child(
        ...
        editor.update(cx, |editor, cx| {
            ...
            assert_eq!(visible[2].entity.read(cx).kind(), BlockType::HtmlBlock);
    ...
    async fn imports_single_line_display_math_between_paragraphs(cx: &mut TestAppContext) {
        ...
        editor.update(cx, |editor, cx| {
            ...
            assert_eq!(visible.len(), 3);
            assert_eq!(visible[0].entity.read(cx).kind(), BlockType::Paragraph);
            assert_eq!(visible[1].entity.read(cx).kind(), BlockType::MathBlock);
    ...
    async fn unclosed_display_math_stays_raw(cx: &mut TestAppContext) {
        ...
        editor.update(cx, |editor, cx| {
            let visible = editor.document.blocks();
            assert_eq!(visible.len(), 1);
            assert_eq!(visible[0].entity.read(cx).kind(), BlockType::RawMarkdown);
            assert_eq!(editor.document.to_markdown(cx), markdown);
        });
    }
    ...
    async fn imports_mermaid_fence_as_native_mermaid_block(cx: &mut TestAppContext) {
        ...
        editor.update(cx, |editor, cx| {
            ...
            assert_eq!(visible.len(), 3);
            assert_eq!(visible[0].entity.read(cx).kind(), BlockType::Paragraph);
            assert_eq!(visible[1].entity.read(cx).kind(), BlockType::MermaidBlock);
            assert_eq!(
                visible[1].entity.read(cx).display_text(),
                "```mermaid\nflowchart LR\nA --> B\n```"
            );
    ...
    async fn imports_tilde_mmd_fence_as_native_mermaid_block(cx: &mut TestAppContext) {
        ...
        editor.update(cx, |editor, cx| {
            let visible = editor.document.blocks();
            assert_eq!(visible.len(), 1);
            assert_eq!(visible[0].entity.read(cx).kind(), BlockType::MermaidBlock);
            assert_eq!(editor.document.to_markdown(cx), markdown);
        });
    }
    ...
    async fn imports_details_html_block_with_blank_lines_as_native_html_block(
        ...
        editor.update(cx, |editor, cx| {
            let visible = editor.document.blocks();
            assert_eq!(visible.len(), 1);
            assert_eq!(visible[0].entity.read(cx).kind(), BlockType::HtmlBlock);
            assert_eq!(visible[0].entity.read(cx).display_text(), markdown);
            assert!(
                visible[0]
                    .entity
    ...
    async fn imports_safe_inline_html_line_as_native_html_block(cx: &mut TestAppContext) {
        ...
        editor.update(cx, |editor, cx| {
            ...
            assert_eq!(block.kind(), BlockType::HtmlBlock);
    ...
    async fn imports_list_items_with_inline_span_style_as_text_not_links(cx: &mut TestAppContext) {
        ...
        editor.update(cx, |editor, cx| {
            ...
            for block in visible {
                assert_eq!(block.entity.read(cx).kind(), BlockType::BulletedListItem);
            }
    ...
    async fn imports_closed_html_comment_as_native_comment_block(cx: &mut TestAppContext) {
        ...
        editor.update(cx, |editor, cx| {
            let visible = editor.document.blocks();
            assert_eq!(visible.len(), 1);
            assert_eq!(visible[0].entity.read(cx).kind(), BlockType::Comment);
            assert_eq!(visible[0].entity.read(cx).display_text(), markdown);
            assert_eq!(editor.document.to_markdown(cx), markdown);
        });
    }
    ...
    async fn html_comment_closes_at_first_marker_and_resumes_block_parsing(
        ...
        editor.update(cx, |editor, cx| {
            ...
            assert_eq!(visible.len(), 3);
            assert_eq!(visible[0].entity.read(cx).kind(), BlockType::Paragraph);
            assert_eq!(visible[1].entity.read(cx).kind(), BlockType::Comment);
            assert_eq!(
                visible[1].entity.read(cx).display_text(),
                "<!--\na\n--> trailing"
            );
    ...
    async fn imports_comment_blocks_inside_list_quote_and_callout(cx: &mut TestAppContext) {
        ...
        quote_editor.update(cx, |editor, cx| {
            ...
            assert_eq!(visible.len(), 3);
            assert_eq!(visible[0].entity.read(cx).kind(), BlockType::Quote);
            assert_eq!(visible[2].entity.read(cx).kind(), BlockType::Comment);
            assert_eq!(
                visible[2].entity.read(cx).display_text(),
                "<!--\nquoted\n-->"
            );
    ...
    async fn parses_multiline_root_footnote_definition_as_native_block(cx: &mut TestAppContext) {
        ...
        editor.update(cx, |editor, cx| {
            ...
            assert_eq!(visible[4].entity.read(cx).kind(), BlockType::Paragraph);
            assert_eq!(
    ...
    async fn test_md_fixture_keeps_mixed_supported_and_raw_sections_visible(
        ...
        editor.update(cx, |editor, cx| {
            ...
            assert!(visible.iter().any(|block| {
                ...
                block.kind() == BlockType::Heading { level: 1 }
                    && block.display_text() == "Markdown Rendering Test Suite"
            }));

            assert!(visible.iter().any(|block| {
                let block = block.entity.read(cx);
                block.kind() == BlockType::Quote
            ...
            assert!(visible.iter().any(|block| {
                let block = block.entity.read(cx);
                block.kind() == BlockType::TaskListItem { checked: false }
            ...
            assert!(visible.iter().any(|block| {
                let block = block.entity.read(cx);
                block.kind() == BlockType::HtmlBlock && block.display_text().contains("<details>")
            }));

            assert!(!visible.iter().any(|block| {
                let block = block.entity.read(cx);
                block.kind() == BlockType::RawMarkdown
    ...
    async fn preserves_reference_definitions_and_stops_quote_at_first_non_quoted_line(
        ...
        quote_editor.update(cx, |editor, cx| {
            ...
            assert_eq!(editor.document.to_markdown(cx), "> quoted\n\ncontinued");
        });
    }
    ...
    async fn imports_blank_line_inside_inline_code_as_single_paragraph(cx: &mut TestAppContext) {
        ...
        editor.update(cx, |editor, cx| {
            ...
            assert_eq!(editor.document.to_markdown(cx), "`line 1\n\nline 2`");
        });
    }
