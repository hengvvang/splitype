...
use crate::workspace::{I18nStrings, StatusBarButton, StatusBarSettings, Theme};
...
impl Editor {
    ...
    pub(crate) fn render_status_bar(
        ...
        if prefs.show_cursor_position && self.view_mode == super::EditMode::Source {
    ...
    pub(crate) fn status_bar_settings(&self, cx: &App) -> StatusBarSettings {
        crate::workspace::EditorSettings::status_bar_settings(cx)
    }
...
fn render_mode_switch(
    ...
    let label = match view_mode {
        super::EditMode::Source => strings.status_bar_mode_rendered.clone(),
        super::EditMode::Wysiwyg => strings.status_bar_mode_source.clone(),
    };
...
mod tests {
    ...
    #[test]
    pub(crate) fn empty_text_has_zero_words() {
        assert_eq!(count_words(""), 0);
    }

    #[test]
    pub(crate) fn english_words_are_counted() {
        assert_eq!(count_words("hello world"), 2);
        assert_eq!(count_words("one two three four"), 4);
    }

    #[test]
    pub(crate) fn cjk_characters_are_counted_individually() {
        assert_eq!(count_words("你好世界"), 4);
        assert_eq!(count_words("中文"), 2);
    }

    #[test]
    pub(crate) fn mixed_cjk_and_english() {
        assert_eq!(count_words("hello 世界"), 3);
        assert_eq!(count_words("你好 world foo"), 4);
    }

    #[test]
    pub(crate) fn whitespace_handling() {
        assert_eq!(count_words("  hello   world  "), 2);
        assert_eq!(count_words("   "), 0);
    }
}
