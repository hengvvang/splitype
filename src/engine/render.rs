...
use crate::blocks::CalloutVariant;
use crate::blocks::switch::Switch;
use crate::blocks::{
    AddLanguageConfig, AddThemeConfig, Block, NoRecentFiles, SelectLanguage, SelectTheme,
};
use crate::shell::chrome::{custom_titlebar_height, render_custom_titlebar};
...
impl Editor {
    ...
    fn render_tiled_layout_node(
        &mut self,
        node: &crate::engine::layout::SplitTree<crate::engine::layout::PaneKind>,
    ...
    fn render_area_tile(
        &mut self,
        leaf_id: usize,
        area_type: crate::engine::layout::PaneKind,
        primary_content: &mut Option<AnyElement>,
        theme: &Theme,
        strings: &I18nStrings,
        leaf_count: usize,
        ...
        let split_h_button = div()
            ...
            .on_click(move |_event, _window, cx| {
                let _ = split_h_editor.update(cx, |ed, cx| {
                    ed.area_layout
                        .split_area(leaf_id, Axis::Horizontal);
                    cx.notify();
                });
            });
    ...
    fn render_area_dropdown_menu(
        &mut self,
        leaf_id: usize,
        current_type: crate::engine::layout::PaneKind,
        theme: &Theme,
        cx: &mut Context<Self>,
    ) -> AnyElement {
        use crate::engine::layout::*;
    ...
    fn render_border_context_menu_overlay(
        &mut self,
        border_menu: crate::engine::layout::BorderMenuState,
        theme: &Theme,
        cx: &mut Context<Self>,
    ) -> AnyElement {
        use crate::engine::layout::*;
                        ...
                        div()
                            ...
                            .on_click(move |_event, _window, cx| {
                                let _ = split_h_ed.update(cx, |ed, cx| {
                                    ed.area_layout
                                        .split_area(split_id, Axis::Horizontal);
                                    cx.notify();
                                });
                            }),
                    )
                        ...
                        div()
                            ...
                            .on_click(move |_event, _window, cx| {
                                let _ = split_v_ed.update(cx, |ed, cx| {
                                    ed.area_layout
                                        .split_area(split_id, Axis::Vertical);
                                    cx.notify();
                                });
                            }),
                    )
    ...
    fn render_empty_panel_prompt(
        colors: &crate::workspace::ThemeColors,
        message: &str,
    ) -> AnyElement {
    ...
    fn render_tiled_edit_container_panel(
        &mut self,
        container_id: usize,
        primary_content: &mut Option<AnyElement>,
        theme: &Theme,
        ...
        let dropdown = if let Some((_cid, inner_id)) = self.area_layout.active_inner_dropdown {
            if _cid == container_id {
                let current_type = self
                    ...
                    .unwrap_or(crate::engine::layout::EditorView::Source);
    ...
    fn render_edit_inner_node(
        &mut self,
        node: &crate::engine::layout::SplitTree<crate::engine::layout::EditorView>,
        container_id: usize,
        primary_content: &mut Option<AnyElement>,
        theme: &Theme,
        strings: &I18nStrings,
    ...
    pub(crate) fn render_inner_area_dropdown_menu(
        ...
        container_id: usize,
        inner_id: usize,
        current_area_type: crate::engine::layout::EditorView,
        theme: &Theme,
        cx: &mut Context<Self>,
    ) -> AnyElement {
        use crate::engine::layout::*;
                        ...
                        .on_click(move |_ev, _win, cx| {
                            let _ = tbl_ed.update(cx, |ed, cx| {
                                ed.area_layout.pref_show_table_headers =
                                    !ed.area_layout.pref_show_table_headers;
                                crate::workspace::EditorSettings::set_show_table_headers(
                                    cx,
                                    ed.area_layout.pref_show_table_headers,
                                );
                                cx.notify();
...
impl Render for Editor {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
                    ...
                    if let Some(footnote_anchor) = row_spacing.footnote_anchor {
                            ...
                            && spacing_for(footnote_end).footnote_anchor == Some(footnote_anchor)
                        {
                            ...
                            let row = div()
                                ...
                                .child(entity.clone());
                            let row = if self.view_mode == super::EditMode::Wysiwyg {
        ...
        if structural_change {
            self.prev_visible_block_ids = blocks
                .iter()
        ...
        let scroll_content = div()
            ...
            .children(block_rows);
        let scroll_content = if self.view_mode == super::EditMode::Wysiwyg {
...
mod tests {
    ...
    use crate::blocks::{AddLanguageConfig, AddThemeConfig};
