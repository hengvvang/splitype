...
| [`identity/`](identity/README.md) | Visual identity — the logo source file plus every derived app icon and banner | `scripts/icon-gen/` |
...
- **Build-time platform resources live in `resources/`**, not here
  (`resources/windows/splitype.ico`, `resources/macos/splitype.icns`,
  `resources/linux/icons/hicolor/…`). `scripts/icon-gen` writes them from the
  identity sources.
