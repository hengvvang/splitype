...
pub fn search_in_source(state: &SourceCodeState, query: &SearchQuery) -> Vec<SearchMatch> {
    let raw_matches = query.find_matches(&state.text, 1);
...
pub fn replace_in_source(state: &mut SourceCodeState, range: Range<usize>, replacement: &str) {
    if range.start <= state.text.len() && range.end <= state.text.len() {
        ...
        state.cursor = range.start + replacement.len();
...
mod tests {
    ...
    fn search_finds_correct_matches_in_source() {
        ...
        let matches = search_in_source(&state, &query);
