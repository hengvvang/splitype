...
impl Editor {
    ...
    pub(crate) fn on_toggle_search(
        &mut self,
        _: &crate::editor_scheduler::commands::actions::ToggleSearch,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) {
        self.toggle_search(window, cx);
    ...
    pub(crate) fn on_toggle_replace(
        &mut self,
        _: &crate::editor_scheduler::commands::actions::ToggleReplace,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) {
    ...
    pub(crate) fn on_find_next(
        &mut self,
        _: &crate::editor_scheduler::commands::actions::FindNext,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) {
    ...
    pub(crate) fn on_find_previous(
        &mut self,
        _: &crate::editor_scheduler::commands::actions::FindPrevious,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) {
    ...
    pub(crate) fn on_replace_current(
        &mut self,
        _: &crate::editor_scheduler::commands::actions::ReplaceCurrent,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) {
    ...
    pub(crate) fn on_replace_all(
        &mut self,
        _: &crate::editor_scheduler::commands::actions::ReplaceAll,
        _window: &mut Window,
        cx: &mut Context<Self>,
    ) {
    ...
    pub fn jump_to_active_search_match(&mut self, window: &mut gpui::Window, cx: &mut Context<Self>) {
        ...
        let pane_kind = self
            ...
            .unwrap_or(crate::editor_scheduler::engine::controller::EditorPaneKind::Wysiwyg);

        if let Some(entity_id) = match_item.entity_id {
            match pane_kind {
                crate::editor_scheduler::engine::controller::EditorPaneKind::Wysiwyg => {
                ...
                crate::editor_scheduler::engine::controller::EditorPaneKind::SourceCode => {
                ...
                crate::editor_scheduler::engine::controller::EditorPaneKind::Preview => {
            ...
            self.request_autoscroll(
                active_pane,
                crate::editor_scheduler::engine::controller::AutoscrollStrategy::Center,
                cx,
            );
            window.refresh();
            cx.notify();
...
use crate::editor_scheduler::engine::controller::EditorView;
