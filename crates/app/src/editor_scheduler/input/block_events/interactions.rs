...
impl Editor {
    pub(crate) fn update_footnote_tooltip(
        ...
        let next = if !show {
            ...
            let (_, pure_content) =
                editor_wysiwyg::markdown::block::footnote::split_footnote_definition_text(&text);
            ...
            self.tab()
                ...
                .map(|entity| {
                    ...
                    let (_, pure_content) =
                        editor_wysiwyg::markdown::block::footnote::split_footnote_definition_text(&plain);
    ...
    pub(crate) fn on_interaction_event(
        ...
        match event {
            BlockEvent::RequestOpenLink {
                ...
                let intent = crate::editor_scheduler::navigation::NavigationIntent::new(
                    crate::editor_scheduler::navigation::NavigationTarget::External {
                        raw: prompt_target.clone(),
                        resolved: open_target.clone(),
                    },
                    crate::editor_scheduler::navigation::NavigationMode::Wysiwyg,
                    modifiers,
                );
            ...
            BlockEvent::RequestJumpToFootnoteDefinition { id, .. } => {
                let intent = crate::editor_scheduler::navigation::NavigationIntent::new(
                    crate::editor_scheduler::navigation::NavigationTarget::FootnoteDefinition { id: id.clone() },
                    crate::editor_scheduler::navigation::NavigationMode::Wysiwyg,
                    Modifiers::default(),
                );
                self.execute_navigation(intent, cx);
            ...
            BlockEvent::RequestJumpToFootnoteBackref { id } => {
                let intent = crate::editor_scheduler::navigation::NavigationIntent::new(
                    crate::editor_scheduler::navigation::NavigationTarget::FootnoteReference { id: id.clone() },
                    crate::editor_scheduler::navigation::NavigationMode::Wysiwyg,
                    Modifiers::default(),
                );
                self.execute_navigation(intent, cx);
