...
impl Editor {
    ...
    pub(crate) fn on_external_paths_drop(
        ...
        let Some(path) =
            crate::editor_scheduler::input::paste::drop::first_dropped_markdown_path(paths.paths())
        else {
            let strings = cx.global::<I18nManager>().strings().clone();
    ...
    pub(crate) fn cancel_drop_replace_dialog(&mut self, cx: &mut Context<Self>) {
        ...
        if let Some(focus_id) = restore_focus {
            ...
            pane.scroll.pending_autoscroll = Some(crate::editor_scheduler::engine::controller::AutoscrollStrategy::Fit {
                margin: px(20.0),
            });
        }
        cx.notify();
