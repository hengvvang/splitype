...
mod tests {
    ...
    fn test_shift_drag_clones_only_the_dragged_leaf() {
        ...
        let result = apply_corner_drag_session(&mut root, &facts, size(px(100.0), px(100.0)));
        let CornerDragResult::CloneWindow {
            source_id,
            container: cloned,
        } = result
        else {
            panic!("Expected CloneWindow result");
        };

        assert_eq!(source_id, split_id);
        let mut ids = Vec::new();
