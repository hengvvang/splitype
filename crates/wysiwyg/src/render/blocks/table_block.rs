...
impl Render for DraggedTableAxisView {
    fn render(&mut self, _window: &mut Window, _cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let axis_len = px(80.0);
        let half_len = px(40.0);
        let thickness = px(2.5);
        let half_thickness = px(1.25);
        ...
        div()
            .relative()
            .size(px(0.0))
            // Horizontal solid crosshair line (80px, 2.5px thickness, centered at drag cursor)
            .child(
                div()
                    .absolute()
                    .left(ox - half_len)
                    .w(axis_len)
                    .top(oy - half_thickness)
                    .h(thickness)
                    .bg(c.table_selection_border),
            )
            // Vertical solid crosshair line (80px, 2.5px thickness, centered at drag cursor)
            .child(
                div()
                    .absolute()
                    .top(oy - half_len)
                    .h(axis_len)
                    .left(ox - half_thickness)
                    .w(thickness)
                    .bg(c.table_selection_border),
            )
    }
...
pub fn render_table(
    ...
    if !cx.has_active_drag() && block.table_axis_preview.is_some() {
        block.table_axis_preview = None;
    }

    let is_dragging_axis = cx.has_active_drag() && block.table_axis_preview.is_some();
    ...
    let row_0_insert_line_bottom = if body_row_count == 0
    ...
    let header_row = div()
        .id(ElementId::Name(
            format!("table-header-row-{}", block.data.id).into(),
        ))
        .relative()
        .w_full()
        .flex()
        ...
        .on_drag_move::<DraggedTableAxis>(move |drag, _window, cx| {
            if drag.drag(cx).table_block_id != block_entity_id
                || drag.drag(cx).kind != TableAxis::Row
            {
                return;
            }
            ...
            let is_horizontally_inside = pos.x >= bounds.origin.x - margin
                && pos.x <= bounds.origin.x + bounds.size.width + margin;
            if !is_horizontally_inside {
                return;
            }

            let y_top = bounds.origin.y;
            let y_bottom = bounds.origin.y + bounds.size.height;
            let y_mid = y_top + bounds.size.height / 2.0;

            if pos.y < y_top - margin {
                return;
            }
            if pos.y >= y_bottom {
                if body_row_count > 0 || pos.y > y_bottom + margin {
                    return;
                }
            }

            let slot = if pos.y < y_mid { 0 } else { 1 };
            let _ = header_drag_move_block.update(cx, |block, cx| {
        ...
        .children(row_0_insert_line_bottom);
        ...
        .map(|(body_row_index, row)| {
            ...
            div()
                ...
                .on_drag_move::<DraggedTableAxis>(move |drag, _window, cx| {
                    ...
                    if pos.y >= y_bottom {
                        if !is_last_body_row || pos.y > y_bottom + margin {
                            return;
                        }
                    }

                    let slot = if pos.y < y_mid {
                        visual_row
                    } else {
                        visual_row + 1
                    };
                    let _ = row_drag_move_block.update(cx, |block, cx| {
                        if block.table_axis_preview
                            != Some(TableAxisMarker {
                                kind: TableAxis::Row,
                                index: slot,
...

    {
        ...
        let table_grid = div()
            ...
            .on_drag_move::<DraggedTableAxis>(move |drag, _window, cx| {
                ...
                let margin = px(24.0);
                let is_inside = pos.x >= bounds.origin.x - margin
                    && pos.x <= bounds.origin.x + bounds.size.width + margin
                    && pos.y >= bounds.origin.y - margin
                    && pos.y <= bounds.origin.y + bounds.size.height + margin;

                if !is_inside {
                    let _ = table_drag_move_block.update(cx, |block, cx| {
                        if block.table_axis_preview.is_some() {
                            block.table_axis_preview = None;
                            cx.notify();
                        }
                    });
            ...
            .on_drop::<DraggedTableAxis>(move |drag, _window, cx| {
                if drag.table_block_id == block_entity_id {
                    ...
                    let _ = table_drop_block.update(cx, |block, cx| {
                        let prev = block.table_axis_preview.take();
