...
impl Block {
    ...
    pub fn sync_code_highlight(&mut self) {
    ...
    pub fn code_language_text(&self) -> &str {
    ...
    pub fn code_language_input_text(&self) -> &str {
    ...
    pub fn code_language_range_to_utf16(&self, range: &Range<usize>) -> Range<usize> {
    ...
    pub fn previous_code_language_boundary(&self, offset: usize) -> usize {
    ...
    pub fn next_code_language_boundary(&self, offset: usize) -> usize {
        self.code_language_input_text()
            .grapheme_indices(true)
            .find_map(|(idx, _)| (idx > offset).then_some(idx))
    ...
    pub fn move_code_language_to(&mut self, offset: usize, cx: &mut Context<Self>) {
    ...
    pub fn select_code_language_to(&mut self, offset: usize, cx: &mut Context<Self>) {
        let clamped = offset.min(self.code_language_input_text().len());
    ...
    pub fn replace_code_language_text_in_range(
        &mut self,
        range: Range<usize>,
        new_text: &str,
    ...
    pub fn choose_code_language(&mut self, value: &str, cx: &mut Context<Self>) {
        if !matches!(
            self.kind(),
    ...
    pub fn code_language_index_for_mouse_position(&self, position: Point<Pixels>) -> usize {
    ...
    pub fn reset_code_language_input_layout(&mut self) {
        self.code_toolbar.picker.paints.clear();
        self.code_toolbar.picker.is_selecting = false;
    }

    pub fn on_code_line_numbers_toggle(
        &mut self,
        _event: &MouseDownEvent,
        _window: &mut Window,
        cx: &mut Context<Self>,
    ) {
        let current = config::settings::PluginSettings::<crate::settings::WysiwygSettings>::get(cx)
            .show_code_line_numbers;
        let _ = config::settings::PluginSettings::<crate::settings::WysiwygSettings>::update(
            cx,
            |settings| {
