...
use markdown_parser::inline::footnote::InlineFootnoteReference;
use markdown_parser::inline::link::InlineLink;
use markdown_parser::inline::render_cache::InlineRenderCache;
use markdown_parser::inline::serialize::can_use_markdown_script_delimiters;
use markdown_parser::inline::style::{InlineScript, InlineStyle};
use markdown_parser::inline::text::{BlockText, InlineAttributes, InlineFragment};

use crate::model::block::CollapsedCaretAffinity;

/// One displayed segment in an expanded inline projection.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct ExpandedInlineSegment {
    pub display_range: Range<usize>,
    pub plain_range: Range<usize>,
    pub fragment_index: usize,
    pub link_group: Option<usize>,
    pub kind: ExpandedInlineSegmentKind,
}

/// Inline construct whose Markdown delimiters can be projected for editing.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum ExpandedInlineKind {
...
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum ExpandedInlineSegmentKind {
...
#[derive(Clone, Debug)]
pub struct ExpandedLinkSpan {
    pub link: InlineLink,
    pub start_fragment_index: usize,
    pub end_fragment_index: usize,
    pub plain_range: Range<usize>,
    pub display_range: Range<usize>,
    pub target_display_range: Range<usize>,
}

/// One projected footnote reference span.
#[derive(Clone, Debug)]
pub struct ExpandedFootnoteSpan {
    pub footnote: InlineFootnoteReference,
    pub plain_range: Range<usize>,
    pub display_range: Range<usize>,
}
...
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct ProjectedLinkSelectionSnapshot {
    pub plain_range: Range<usize>,
    pub display_relative_range: Range<usize>,
    pub selection_reversed: bool,
}

/// Render cache and offset maps for an expanded inline projection.
#[derive(Clone, Debug)]
pub struct ExpandedInlineProjection {
    pub cache: InlineRenderCache,
    pub segments: Vec<ExpandedInlineSegment>,
    pub block_prefix_range: Option<Range<usize>>,
    pub plain_to_display_cursor: Vec<usize>,
    pub display_to_plain: Vec<usize>,
    pub link_spans: Vec<ExpandedLinkSpan>,
    pub footnote_spans: Vec<ExpandedFootnoteSpan>,
}
...
impl ExpandedInlineProjection {
    ...
    pub fn caret_closes_span_at_plain(&self, plain: usize) -> bool {
    ...
    pub fn display_offset_for_plain_cursor(
        &self,
        plain: usize,
        ...
        match affinity {
            CollapsedCaretAffinity::Default => {
                for span in &self.footnote_spans {
                    if span.plain_range.end == plain {
                        return Some(span.display_range.end.saturating_sub(1));
                    }
                    if span.plain_range.start == plain {
                        return Some(span.display_range.start + 2);
                    }
                }
    ...
    pub fn link_span_fully_covering_range(
        &self,
        range: &Range<usize>,
    ...
    pub fn link_span_for_plain_range(
        &self,
    ...
    pub fn footnote_span_fully_covering_range(
        &self,
        range: &Range<usize>,
    ...
    pub fn delimiter_ranges(&self) -> Vec<Range<usize>> {
