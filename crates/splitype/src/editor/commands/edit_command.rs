...
impl Editor {
    ...
    pub(crate) fn apply_line_prefix(
        &mut self,
        target_entity: Option<EntityId>,
        prefix: &str,
        cx: &mut Context<Self>,
        ...
        block.update(cx, |block, cx| {
            if !block.edits_verbatim_text() {
                block.data.kind = splitype_model::parse::BlockKind::Paragraph;
            }
            let cursor = block.cursor_offset();
    ...
    pub(crate) fn apply_clear_format(
        &mut self,
        target_entity: Option<EntityId>,
        cx: &mut Context<Self>,
    ) {
...
pub(crate) fn strip_markdown_line_prefix(line: &str) -> &str {
...
#[cfg(test)]
mod tests {
    use super::{strip_markdown_line_prefix, BlockStructureKind, InlineFormatKind};

    #[test]
    fn test_strip_markdown_line_prefix() {
        ...
        assert_eq!(strip_markdown_line_prefix("1. Numbered item"), "Numbered item");
        assert_eq!(strip_markdown_line_prefix("- [ ] Task item"), "Task item");
        assert_eq!(strip_markdown_line_prefix("- [x] Completed task"), "Completed task");
        assert_eq!(strip_markdown_line_prefix("> Quote line"), "Quote line");
        assert_eq!(strip_markdown_line_prefix("Plain line"), "Plain line");
    }

    #[test]
    fn test_inline_format_kind_delimiters_and_templates() {
        assert_eq!(InlineFormatKind::Bold.empty_template(), ("****", 2));
        assert_eq!(InlineFormatKind::Bold.wrap_delimiters(), ("**", "**"));

        assert_eq!(InlineFormatKind::Italic.empty_template(), ("**", 1));
        assert_eq!(InlineFormatKind::Italic.wrap_delimiters(), ("*", "*"));

        assert_eq!(InlineFormatKind::Strikethrough.empty_template(), ("~~~~", 2));
        assert_eq!(InlineFormatKind::Strikethrough.wrap_delimiters(), ("~~", "~~"));

        assert_eq!(InlineFormatKind::Highlight.empty_template(), ("====", 2));
        assert_eq!(InlineFormatKind::Highlight.wrap_delimiters(), ("==", "=="));

        assert_eq!(InlineFormatKind::InlineCode.empty_template(), ("``", 1));
        assert_eq!(InlineFormatKind::InlineCode.wrap_delimiters(), ("`", "`"));

        assert_eq!(InlineFormatKind::InlineMath.empty_template(), ("$$", 1));
        assert_eq!(InlineFormatKind::InlineMath.wrap_delimiters(), ("$", "$"));

        assert_eq!(InlineFormatKind::Comment.empty_template(), ("<!---->", 4));
        assert_eq!(InlineFormatKind::Comment.wrap_delimiters(), ("<!--", "-->"));

        assert_eq!(InlineFormatKind::ClearFormat.empty_template(), ("", 0));
        assert_eq!(InlineFormatKind::ClearFormat.wrap_delimiters(), ("", ""));
    }

    #[test]
    fn test_block_structure_kind_prefixes() {
        assert_eq!(BlockStructureKind::Heading(1).source_prefix(), "# ");
        assert_eq!(BlockStructureKind::Heading(3).source_prefix(), "### ");
        assert_eq!(BlockStructureKind::Heading(6).source_prefix(), "###### ");
        assert_eq!(BlockStructureKind::Paragraph.source_prefix(), "");
        assert_eq!(BlockStructureKind::BulletList.source_prefix(), "- ");
        assert_eq!(BlockStructureKind::NumberedList.source_prefix(), "1. ");
        assert_eq!(BlockStructureKind::TaskList.source_prefix(), "- [ ] ");
        assert_eq!(BlockStructureKind::Blockquote.source_prefix(), "> ");
    }
}
