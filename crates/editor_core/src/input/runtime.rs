...
impl Editor {
    pub fn current_edit_target_entity_id_from_state(&self, cx: &App) -> Option<EntityId> {
    ...
    pub fn current_edit_target_from_state(&self, cx: &App) -> Option<Entity<Block>> {
        self.current_edit_target_entity_id_from_state(cx)
            .and_then(|entity_id| self.focusable_entity_by_id(entity_id))
    }

    pub fn end_block_pointer_selection_sessions_inner(
        &mut self,
        cx: &mut Context<Self>,
        notify: bool,
    ) -> bool {
    ...
    pub fn end_block_pointer_selection_sessions(&mut self, cx: &mut Context<Self>) -> bool {
    ...
    pub fn new_block(&mut self, cx: &mut Context<Self>, data: BlockData) -> Entity<Block> {
    ...
    pub fn rebuild_footnote_registry(&mut self, cx: &App) -> FootnoteMap {
    ...
    pub fn sync_references_after_block_change(
        &mut self,
        block: &Entity<Block>,
        cx: &mut Context<Self>,
    ) {
    ...
    pub fn rebuild_reference_registries(&mut self, cx: &mut Context<Self>) {
        use std::sync::Arc;
    ...
    pub fn focusable_entity_by_id(&self, entity_id: EntityId) -> Option<Entity<Block>> {
    ...
    pub fn first_focusable_entity_id(&self, cx: &App) -> Option<EntityId> {
    ...
    pub fn focused_edit_target_entity_id(
        &self,
        window: &Window,
        cx: &App,
    ) -> Option<EntityId> {
    ...
    pub fn focused_edit_target(&self, window: &Window, cx: &App) -> Option<Entity<Block>> {
    ...
    pub fn table_block_by_id(&self, entity_id: EntityId, cx: &App) -> Option<Entity<Block>> {
        self.doc()
