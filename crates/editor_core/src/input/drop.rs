...
impl Editor {
    ...
    pub(crate) fn replace_document_from_markdown(
        ...
        self.tab_mut().text = normalized.clone();
        self.tab_mut().document = None;
        self.tab_mut().text_stale = false;
        self.tab_mut().document_revision = self.tab_mut().document_revision.wrapping_add(1);
        ...
        for pane_id in pane_ids {
            ...
            pane.scroll.pending_autoscroll = Some(crate::engine::controller::AutoscrollStrategy::Top {
                margin: px(0.0),
            });
