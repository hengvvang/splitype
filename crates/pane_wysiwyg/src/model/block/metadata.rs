...
pub const FLAG_QUOTE_CONTAINER: u8 = 1 << 0;
pub const FLAG_LIST_ITEM: u8 = 1 << 1;
pub const FLAG_FOOTNOTE_DEF: u8 = 1 << 2;
pub const FLAG_CALLOUT: u8 = 1 << 3;

impl Block {
    pub fn take_quote_reparse_requested(&mut self) -> bool {
        let requested = self.quote_reparse_requested;
        self.quote_reparse_requested = false;
        requested
    }

    pub fn take_numbered_list_restart_requested(&mut self) -> bool {
        let requested = self.numbered_list_restart_requested;
        self.numbered_list_restart_requested = false;
        requested
    }

    pub fn kind_metadata_flags(&self) -> u8 {
        let mut flags = 0u8;
    ...
    pub fn is_tree_metadata_current(&self) -> bool {
