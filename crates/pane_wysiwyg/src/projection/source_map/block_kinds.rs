...
pub fn push_inline_block_mapping(
    ...
    let (full_text, content_to_source, source_to_content) =
        build_prefixed_content_mapping(&content_markdown, &first_prefix, &continuation_prefix);
    let (full_text, content_to_source, source_to_content) = wrap_source_mapping_with_quotes(
        full_text,
        content_to_source,
        source_to_content,
        quote_depth,
...
pub fn push_footnote_definition_full_mapping(
    block: &Entity<Block>,
    footnote_id: &str,
    first_line: &str,
    quote_depth: usize,
...
pub fn push_raw_block_mapping(
    ...
    let (full_text, content_to_source, source_to_content) =
        build_prefixed_content_mapping(&content, &indentation, &indentation);
    let (full_text, content_to_source, source_to_content) = wrap_source_mapping_with_quotes(
        full_text,
        content_to_source,
        source_to_content,
...
/// Pushes the source mapping for a fenced block (math or Mermaid).
pub fn push_fenced_block_mapping(
    block: &Entity<Block>,
    quote_depth: usize,
    absolute_start: usize,
    mappings: &mut Vec<SourceTargetMapping>,
    ...
    let (full_text, serialized_to_full, full_to_serialized) =
        build_prefixed_content_mapping(&serialized, &indentation, &indentation);
...
pub fn push_code_block_mapping(
    block: &Entity<Block>,
    quote_depth: usize,
    absolute_start: usize,
    mappings: &mut Vec<SourceTargetMapping>,
    ...
    let (full_text, content_to_source, source_to_content) =
        build_code_block_content_mapping(&content, &indentation, language.as_deref());
    let (full_text, content_to_source, source_to_content) = wrap_source_mapping_with_quotes(
        full_text,
        content_to_source,
        source_to_content,
...
pub fn wrap_source_mapping_with_quotes(
    ...
    for _ in 0..quote_depth {
        let (wrapped_text, inner_to_wrapped, wrapped_to_inner) =
            build_prefixed_content_mapping(&full_text, "> ", "> ");
