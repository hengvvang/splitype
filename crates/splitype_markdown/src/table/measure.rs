...
//! presentation logic lives in `editor_model::geometry` instead of the pure
//! `crate::block::table` data model. The layout *math*
...
//! `TableColumnLayout`) because the type is defined in `crate::block::table`.

use gpui::{FontStyle, FontWeight, Pixels, SharedString, TextRun, Window, px};

use theme::Theme;
use crate::block::table::{TableColumnLayout, TableData};
...
pub fn measure_table_column_layout(
    table: &TableData,
    table_width: f32,
    window: &mut Window,
    theme: &Theme,
