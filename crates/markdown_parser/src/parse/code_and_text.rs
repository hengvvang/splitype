...
pub(crate) fn collect_fenced_code_block<S: AsRef<str>>(
    lines: &[S],
    start: usize,
) -> Option<(BlockData, usize)> {
    let fence = parse_opening_fence(lines[start].as_ref())?;
    ...
    if is_mermaid_info_string(fence.language.as_ref().map(|language| language.as_ref())) {
        let raw = lines[start..=closing_index]
            .iter()
...
pub(crate) fn collect_indented_code_block<S: AsRef<str>>(
    lines: &[S],
    start: usize,
) -> Option<(BlockData, usize)> {
    let stripped = strip_indented_code_prefix(lines[start].as_ref())?;
    let mut code_lines = vec![stripped.to_string()];
    let mut code_index = start + 1;
    while code_index < lines.len() {
        if let Some(stripped) = strip_indented_code_prefix(lines[code_index].as_ref()) {
            code_lines.push(stripped.to_string());
            code_index += 1;
        } else if lines[code_index].as_ref().trim().is_empty() {
...
pub(crate) fn collect_comment_block<S: AsRef<str>>(
    lines: &[S],
    start: usize,
) -> Option<(BlockData, usize)> {
...
pub(crate) fn collect_paragraph_block<S: AsRef<str>>(
    lines: &[S],
    start: usize,
) -> (BlockData, usize) {
    let mut paragraph_lines = vec![lines[start].as_ref()];
    let mut index = start + 1;
    while index < lines.len() {
        if (lines[index].as_ref().trim().is_empty() || looks_like_root_block_start(lines, index))
            && !paragraph_can_continue_through_boundary(&paragraph_lines, lines, index)
        {
            break;
        }
        paragraph_lines.push(lines[index].as_ref());
        index += 1;
    }
