...
use crate::markdown::block::html::{FenceInfo, HtmlBlockStart};
...
#[derive(Clone, Debug, PartialEq, Eq)]
pub enum TableCellInlineImageSegment {
    Text(String),
    Image {
        markdown: String,
        syntax: ImageSyntax,
...
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct ImageReferenceDefinition {
    pub src: String,
    pub title: Option<String>,
}

pub type ImageReferenceDefinitions = HashMap<String, ImageReferenceDefinition>;

/// Resolved image target ready for path or URL loading.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct ResolvedImageTarget {
    pub src: String,
    pub title: Option<String>,
}
...
pub fn parse_standalone_image(markdown: &str) -> Option<ImageSyntax> {
...
pub fn parse_table_cell_inline_images(markdown: &str) -> Vec<TableCellInlineImageSegment> {
...
pub fn parse_image_reference_definitions(markdown: &str) -> ImageReferenceDefinitions {
...
pub fn normalize_reference_label(label: &str) -> Option<String> {
...
mod tests {
    ...
    fn parses_standalone_image_without_title() {
        let syntax = parse_standalone_image("![alt](image.png)").expect("image");
        assert_eq!(syntax.alt, "alt");
        assert_eq!(
            syntax.target,
            ImageTarget::Direct {
    ...
    fn parses_standalone_image_with_surrounding_whitespace() {
        let syntax = parse_standalone_image("  ![logo](icon.svg)  ").expect("image");
        assert_eq!(syntax.alt, "logo");
        assert_eq!(
            syntax.target,
            ImageTarget::Direct {
    ...
    fn parses_image_target_with_escaped_punctuation_in_source() {
        let syntax = parse_standalone_image(r"![alt](path\/to\/file.png)").expect("image");
        assert_eq!(syntax.alt, "alt");
        assert_eq!(
            syntax.target,
            ImageTarget::Direct {
                src: "path/to/file.png".to_string(),
                title: None
            }
        );
    }
    ...
    fn parses_standalone_image_with_underscores_in_alt_and_source() {
        let syntax = parse_standalone_image("![alt_text](path/image_name.png)").expect("image");
        assert_eq!(syntax.alt, "alt_text");
        assert_eq!(
            syntax.target,
            ImageTarget::Direct {
                src: "path/image_name.png".to_string(),
                title: None
            }
        );
    }
    ...
    fn parses_standalone_image_with_title() {
        let syntax = parse_standalone_image(r#"![alt](image.png "A title")"#).expect("image");
        assert_eq!(syntax.alt, "alt");
        assert_eq!(
            syntax.target,
            ImageTarget::Direct {
    ...
    fn parses_reference_style_standalone_image() {
        let syntax = parse_standalone_image("![alt][ref]").expect("image");
        assert_eq!(syntax.alt, "alt");
        assert_eq!(
            syntax.target,
    ...
    fn parses_collapsed_reference_style_standalone_image() {
        let syntax = parse_standalone_image("![alt][]").expect("image");
        assert_eq!(syntax.alt, "alt");
        assert_eq!(
            syntax.target,
            ImageTarget::Reference {
    ...
    fn rejects_mixed_or_wrapped_image_syntax() {
        assert!(parse_standalone_image("![alt](src)[ref]").is_none());
    }
    ...
    fn parses_table_cell_inline_image_segments() {
        ...
        match &segments[1] {
            TableCellInlineImageSegment::Image { markdown, syntax } => {
                assert_eq!(markdown, "![img](a.png)");
                assert_eq!(syntax.alt, "img");
            }
    ...
    fn parses_multiple_table_cell_inline_images() {
        let segments = parse_table_cell_inline_images("![a](1.png) and ![b](2.png)");
        let image_count = segments
            .iter()
            .filter(|s| matches!(s, TableCellInlineImageSegment::Image { .. }))
            .count();
    ...
    fn table_cell_inline_image_segments_keep_escaped_wrapped_and_broken_text() {
        let segments = parse_table_cell_inline_images(r"escaped \!\[not an image\] text");
    ...
    fn table_cell_inline_reference_images_resolve() {
        let refs = {
            let mut map = ImageReferenceDefinitions::new();
            map.insert(
                "ref".to_string(),
                ImageReferenceDefinition {
        ...
        let resolved = syntax.resolve_target(&refs).expect("resolved");
        assert_eq!(resolved.src, "resolved.png");
        assert_eq!(resolved.title, Some("resolved title".to_string()));
    }

    #[test]
    fn parses_image_reference_definitions_with_title_and_first_wins() {
        let markdown = r#"[logo]: image.png "Logo title""#;
        let definitions = parse_image_reference_definitions(markdown);
        let def = definitions.get("logo").expect("definition");
        assert_eq!(def.src, "image.png");
        assert_eq!(def.title, Some("Logo title".to_string()));
    }

    #[test]
    fn normalizes_reference_labels_case_and_whitespace_insensitively() {
        assert_eq!(
            normalize_reference_label("My Label"),
            Some("my label".to_string())
        );
        assert_eq!(
            normalize_reference_label("  multi   word  "),
    ...
    fn resolves_reference_targets() {
        let mut refs = ImageReferenceDefinitions::new();
        refs.insert(
            "ref".to_string(),
            ImageReferenceDefinition {
                src: "resolved.png".to_string(),
    ...
    fn resolves_collapsed_and_shortcut_reference_images() {
        ...
        let syntax = parse_standalone_image("![alt][]").expect("collapsed");
        let resolved = syntax.resolve_target(&refs).expect("resolved");
        assert_eq!(resolved.src, "image.png");

        let syntax = parse_standalone_image("![alt]").expect("shortcut");
        let resolved = syntax.resolve_target(&refs).expect("resolved");
        assert_eq!(resolved.src, "image.png");
    }
    ...
    fn unresolved_reference_target_returns_none() {
        let refs = ImageReferenceDefinitions::new();
        let syntax = ImageSyntax {
            alt: "alt".to_string(),
            target: ImageTarget::Reference {
        ...
        assert!(syntax.resolve_target(&refs).is_none());
    }

    #[test]
    fn resolves_relative_and_remote_sources() {
        let base = Path::new("/project/docs");
        let source = resolve_image_source("assets/img.png", Some(base));
        assert!(matches!(source, ImageResolvedSource::Local(_)));

        let source = resolve_image_source("https://example.com/image.png", None);
        assert!(matches!(source, ImageResolvedSource::Remote(_)));
    }
    ...
    fn parses_container_scoped_reference_definitions_in_source_order() {
        ...
        let definitions = parse_image_reference_definitions(markdown);
        assert_eq!(definitions.get("a").map(|d| d.src.as_str()), Some("a.png"));
    }
