//! Link reference definitions for reference-style Markdown links.
//!
//! Reference definitions appear as `[label]: url "title"` lines
//! and are collected into a document-wide map.

use std::collections::HashMap;

use crate::markdown::block::html::{FenceInfo, HtmlBlockStart};
use crate::markdown::block::image::normalize_reference_label;
...
/// Document-wide map from reference label to definition.
pub type LinkReferenceDefinitions = HashMap<String, LinkReferenceDefinition>;

pub fn parse_link_reference_definitions(markdown: &str) -> LinkReferenceDefinitions {
...
pub fn is_supported_autolink_target(target: &str) -> bool {
    if target
        .strip_prefix("mailto:")
...
pub fn parse_link_target(inner: &str) -> Option<(String, Option<String>)> {
    if inner.is_empty() {
        return None;
    }
