//! Rendered standalone image handle state on [`Block`].
...
impl Block {
    ...
    pub fn can_present_as_image(&self) -> bool {
        self.is_table_cell()
            || matches!(
                self.kind(),
                BlockKind::Paragraph
    ...
    pub fn image_base_dir(&self) -> Option<&Path> {
        self.image_base_dir.as_deref()
    }

    pub fn sync_image_handle(&mut self) -> bool {
    ...
    pub fn standalone_image_markdown_for_handle(&self) -> Option<String> {
    ...
    pub fn is_showing_rendered_image(&self) -> bool {
