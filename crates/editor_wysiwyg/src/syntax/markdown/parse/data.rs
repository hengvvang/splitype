...
use crate::markdown::block::html::{HtmlDocument, parse_html_document};
    ...
    pub fn serialize_markdown_line(&self, depth: usize, list_ordinal: Option<usize>) -> String {
        ...
        match self.kind {
            ...
            BlockKind::Blockquote => prefixed_multiline(
                &crate::markdown::block::CalloutKind::escape_plain_quote_header(&text_markdown),
                &format!("{indentation}> "),
                &format!("{indentation}> "),
            ),
            ...
            BlockKind::FootnoteDefinition => {
                let (id, content) =
                    crate::markdown::block::footnote::split_footnote_definition_text(&text_markdown);
                if content.is_empty() {
            ...
            BlockKind::MathBlock | BlockKind::MermaidBlock => {
                ...
                let (source, _) = match self.kind {
                    ...
                    _ => crate::markdown::block::mermaid::serialize_mermaid_source(&body),
                };
                if depth == 0 {
                    source
                } else {
...
impl crate::tree::Item for BlockData {
    type Summary = crate::tree::BlockSummary;
    fn summary(&self, _cx: &()) -> crate::tree::BlockSummary {
        let plain = self.text.plain_text();
        ...
        let base_height = match self.kind {
            crate::markdown::parse::kind::BlockKind::Heading { level } => match level {
                1 => 44.0,
                2 => 36.0,
                3 => 30.0,
                _ => 26.0,
            },
            crate::markdown::parse::kind::BlockKind::CodeBlock { .. } => 24.0 * line_count as f32 + 32.0,
            crate::markdown::parse::kind::BlockKind::MathBlock | crate::markdown::parse::kind::BlockKind::MermaidBlock => 80.0,
            crate::markdown::parse::kind::BlockKind::Table => 28.0 * line_count as f32 + 32.0,
            _ => 24.0 * line_count as f32,
        };
        crate::tree::BlockSummary {
            total_blocks: 1,
